...
reformer_llm = ChatOpenAI(model_name="gpt-4o-mini", openai_api_key=OPENAI_API_KEY)
responder_llm = ChatOpenAI(model_name="gpt-4o", openai_api_key=OPENAI_API_KEY, temperature=0.5)
...
# 검색용 질문 생성 함수
def generate_search_query(chat_history, question):
    history_text = "\n".join([f"Q: {q}\nA: {a}" for q, a in chat_history])
    prompt = f"""
    아래는 AI와 사용자 간의 대화입니다.
    마지막 질문의 의미를 정확하게 파악하고, 앞선 대화 맥락을 반영한 검색용 질문(query)을 생성하세요.
    - 특정 상품을 지칭 할 경우 정확한 상품명을 검색어로 넣으세요
    - chroma db에서 검색 정확성을 높일수 있도도록 정확한 단어를 사용하고 쓸데없는 단어를 배제하세요.
    ...
    ### 검색용 질문 ###
    """
    return reformer_llm.predict(prompt).strip()

# QA 체인 생성 함수 수정

def get_dual_llm_qa_chain(db):
    ...
    def custom_run(query):
        # 세션 기록이 있으면 검색용 질문 생성
        chat_history = st.session_state.get("history", [])
        search_query = generate_search_query(chat_history, query)
        ...
        for doc in retrieved_docs:
            ...
            bank_products[bank_name].append(doc.page_content[:700])
...
def main():
    ...
    qa_chain = get_dual_llm_qa_chain(db)


    user_input = st.text_input("질문을 입력하세요:", key="input_box")
    ask_button = st.button("질문하기")
    
    if user_input and (ask_button or user_input != st.session_state.get("last_input", "")) and (user_input not in ["대화 종료"]):
