...
목적:
- 사용자의 금융 정보를 입력하면 적절한 대출 상품이 추천되는지 확인한다.

- 대출 한도 및 금리가 정확하게 계산되고 추천 리스트가 올바르게 출력되는지 검증한다.
...
- 시스템이 사용자의 정보를 분석하고, 적절한 대출 상품을 추천하는지 확인한다.

- 추천된 대출 상품 목록이 다음 기준에 부합하는지 확인한다.

- 대출 한도가 사용자의 소득과 신용등급을 반영한 적절한 범위인지

-   금리가 합리적인 수준으로 설정되었는지

- 상환 기간이 사용자의 요구와 일치하는지
